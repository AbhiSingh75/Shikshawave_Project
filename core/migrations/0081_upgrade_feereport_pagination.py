...
class Migration(migrations.Migration):
    """
    Upgrade Proc_FeeReport_get: 
    1. Adds pagination parameters (mint_PageSize, mint_PageIndex).
    2. Adds dynamic sorting parameters (mvar_SortColumn, mvar_SortOrder).
    3. Returns 'TotalRecords' in the overview result set for pagination math.
    """

    dependencies = [
        ('core', '0080_fix_feereport_column_aliases'),
    ]
...
DROP FUNCTION IF EXISTS "Proc_FeeReport_get"(INTEGER, DATE, DATE, INTEGER, INTEGER, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, INTEGER);
...
$$ LANGUAGE plpgsql;
            """,
            reverse_sql="DROP FUNCTION IF EXISTS \"Proc_FeeReport_get\"(INTEGER, DATE, DATE, INTEGER, INTEGER, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER, INTEGER, VARCHAR, VARCHAR);"
        ),
    ]
