...
BEGIN
    ...
    -- Return activity log (exclude comments)
    SELECT 
        a.ActivityID,
        a.ActionType,
        a.OldStatus,
    ...
    WHERE a.TicketID = @TicketID AND a.ActionType != 'Commented'
    ORDER BY a.Timestamp DESC;
    
    -- Return comments with attachment info (sorted ascending for chat)
    SELECT 
        c.CommentID,
        c.CommentByUserID,
        c.CommentText,
        ...
        rc.CommentText AS ReplyToText,
        ru.UserName AS ReplyToUserName
    FROM TicketComments c
    INNER JOIN UserMaster u ON c.CommentByUserID = u.UserID
    LEFT JOIN TicketComments rc ON c.ReplyToCommentID = rc.CommentID
    LEFT JOIN UserMaster ru ON rc.CommentByUserID = ru.UserID
