...
BEGIN
    ...
    SELECT 
        ...
        AVG(CASE WHEN ResolvedAt IS NOT NULL THEN DATEDIFF(HOUR, CreatedAt, ResolvedAt) ELSE NULL END) AS AvgResolutionTimeHours,
    ...
    LEFT JOIN TicketMaster t ON t.CategoryID = c.CategoryID
        AND t.IsDeleted = 0
    ...
    SELECT 
        ...
        CAST(COUNT(*) * 100.0 / NULLIF((SELECT COUNT(*) FROM TicketMaster WHERE IsDeleted = 0 AND CAST(CreatedAt AS DATE) BETWEEN @StartDate AND @EndDate AND (@RoleName = 'Super Admin' OR SchoolID = @UserSchoolID) AND (@RoleName != 'Support Executive' OR AssignedToUserID = @UserID)), 0) AS DECIMAL(5,2)) AS Percentage
    FROM TicketMaster
    WHERE IsDeleted = 0
        AND CAST(CreatedAt AS DATE) BETWEEN @StartDate AND @EndDate
        AND (@RoleName = 'Super Admin' OR SchoolID = @UserSchoolID)
    ...
    BEGIN
        SELECT TOP 5
            ...
            AVG(CASE WHEN t.ResolvedAt IS NOT NULL THEN DATEDIFF(HOUR, t.CreatedAt, t.ResolvedAt) ELSE NULL END) AS AvgResolutionTimeHours
        FROM TicketMaster t
...
PRINT 'Proc_Ticket_Insights_Dashboard updated successfully!';
GO
